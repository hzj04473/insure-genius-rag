...
class EnhancedPDFProcessor:
    ...
    def process_company_documents(self, company_name: str) -> bool:
        ...
        print(
            f"🎉 {company_name} 처리 완료: {processed_files}/{len(pdf_files)} 파일, 총 {total_chunks} 청크"
        )
    ...
    def search_company_clauses(
        self, query: str, company_name: str = None, top_k: int = 5
    ) -> List[Dict]:
        """특정 보험사나 전체에서 약관 검색"""
...
def search_similar_clauses(
    query: str, company_name: str = None, top_k: int = 5
) -> List[Dict]:
    """기존 코드와의 호환성을 위한 함수"""
...
def process_pdf_to_pinecone(
    pdf_path: str, company_name: str, document_type: str = None
):
    """단일 PDF 처리 함수"""
    processor = EnhancedPDFProcessor()
